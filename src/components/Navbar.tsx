...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const handleNotificationClick = (item: any) => {
    ...
    if (judul.includes('teman') || pesan.includes('teman sejawat') || pesan.includes('antar teman')) {
      onSelectMenuItem(isMurid ? 'penilaian-teman-saya' : 'penilaian-teman', targetId);
      return;
    }
          ...
          >
            {syncStatus === 'disabled' ? (
              <>
                ...
                <span className="hidden md:inline font-bold text-[11px]">Firebase Nonaktif</span>
              </>
            ) : syncStatus === 'synced' ? (
              <>
                <span className="relative flex h-2 w-2 shrink-0">
          ...
          {showSyncDetails && (
            <>
              ...
              <div className="fixed inset-x-3 top-20 max-w-sm mx-auto sm:max-w-none sm:inset-auto sm:absolute sm:right-0 sm:top-full sm:mt-2 sm:w-84 bg-white border border-gray-200 rounded-2xl shadow-2xl z-50 p-4 text-xs text-slate-700 animate-in fade-in zoom-in-95 duration-150">
                  ...
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h4 className="font-extrabold text-slate-900 text-sm truncate">
                        {syncStatus === 'disabled' ? 'Penyimpanan Lokal' : 'Cloud Firestore'}
                      </h4>
                      <p className="text-[10px] text-slate-500 truncate">
                        {syncStatus === 'disabled' ? (
                          <span className="font-semibold text-slate-500">Firebase Dinonaktifkan</span>
                        ) : (
                          <>
                            Terakhir sinkron:{' '}
                ...
                {/* Konten Popover: Jika Disabled vs Jika Aktif */}
                {syncStatus === 'disabled' ? (
                  <div className="space-y-3">
                    <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1.5">
                      <div className="flex items-center gap-2 text-slate-800 font-bold text-xs">
                        <CloudOff className="w-4 h-4 text-slate-500 shrink-0" />
                        <span>Mode Lokal (Offline-First)</span>
                      </div>
                      <p className="text-[11px] text-slate-600 leading-relaxed">
                        Firebase saat ini dinonaktifkan. Semua data murid, tugas, nilai, kuis, dan absensi tersimpan secara aman di penyimpanan internal peramban (LocalStorage).
                      </p>
                    </div>

                    <button
                      type="button"
                      id="btn-enable-firebase"
                      onClick={() => {
                        dataStorage.setFirebaseEnabled(true);
                        setSyncFeedbackMsg('Sinkronisasi Firebase diaktifkan');
                        setTimeout(() => setSyncFeedbackMsg(null), 3500);
                      }}
                      className="w-full py-2.5 px-3.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-colors text-xs shadow-xs cursor-pointer"
                    >
                      <Cloud className="w-4 h-4" />
                      <span>Aktifkan Sinkronisasi Firebase</span>
                    </button>
                  </div>
                ) : (
                  <div className="space-y-2">
                    ...
                    <div className="pt-2 border-t border-gray-100">
                      <button
                        type="button"
                        id="btn-disable-firebase"
                        onClick={() => {
                          dataStorage.setFirebaseEnabled(false);
                          setSyncFeedbackMsg('Firebase berhasil dinonaktifkan');
                          setTimeout(() => setSyncFeedbackMsg(null), 3500);
                        }}
                        className="w-full py-2 text-slate-500 hover:text-rose-600 hover:bg-rose-50 rounded-xl font-bold flex items-center justify-center gap-1.5 transition-colors text-xs cursor-pointer"
                      >
                        <CloudOff className="w-3.5 h-3.5 text-rose-500" />
                        <span>Nonaktifkan Sinkronisasi Firebase</span>
                      </button>
                    </div>
                  </div>
                )}
